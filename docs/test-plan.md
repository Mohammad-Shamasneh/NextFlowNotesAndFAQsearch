...
| T02 | search_notes | Keep the normal Markdown fixtures in `data/`. | Use the valid input from `examples/search_notes.json`. | Returns matching note results and snippets while respecting the configured result limit. | PASS | Screenshot: MCP Inspector output for `search_notes` |
...
| T05 | search_notes | Start the MCP server normally with the standard fixtures. | `{ "query": "" }` | Rejects the empty search query through input validation and returns a clean validation error. | PASS | Screenshot: MCP Inspector validation error |
...
| T07 | list_notes | Temporarily move or rename the Markdown fixtures so that `data/` contains no note files. Restore all fixtures after the test. | `{}` | Returns an empty file list with a short message and does not crash. | PASS | Screenshot: MCP Inspector empty `data/` result |
